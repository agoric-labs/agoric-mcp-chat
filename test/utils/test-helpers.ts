 ...
 * Handles both "data:" format and AI SDK "0:" format
 */
export function parseStreamingChunks(chunks: string[]): any[] {
  ...
  for (const line of lines) {
    // Handle "data: {...}" format (standard SSE)
 ...
 * Handles both event.delta and event.textDelta formats
 */
export function extractTextFromEvents(events: any[]): string {
  ...
  for (const event of events) {
    if (event.type === 'text-delta') {
      // Handle both delta and textDelta properties
