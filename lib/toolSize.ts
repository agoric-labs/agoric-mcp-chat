 ...
 */
export function checkToolSchemaSize(
  tools: Record<string, any>,
): ToolSchemaSizeResult {
  ...
  if (estimatedTokens > TOKEN_ERROR_THRESHOLD) {
    console.error(
      `Schema too large: ~${estimatedTokens} tokens (${toolCount} tools)`,
    );
    return {
      estimatedTokens,
      toolCount,
  ...
  if (estimatedTokens > TOKEN_WARNING_THRESHOLD) {
    console.warn(
      `Schema approaching limit: ~${estimatedTokens} tokens (${toolCount} tools)`,
    );
    return {
      estimatedTokens,
      toolCount,
  ...
  console.log(
    `Tools loaded: ${toolCount} tools, ~${Math.round(estimatedTokens / 1000)}k tokens estimated`,
  );
  return {
    estimatedTokens,
    toolCount,
