...
export const agoricMcpDevopsToolSchemas = {
    ...
    'render-webpage': {
        inputSchema: z.object({
            ...
            waitFor: z.number().optional().describe('Time to wait in milliseconds before capturing (default: 5000)'),
            selector: z.string().optional().describe('CSS selector to wait for before capturing'),
        }),
    },
    ...
    'fetch-information-from-vstorage': {
        inputSchema: z.object({
            path: z
                .string()
                .describe("The vstorage path to fetch data from (e.g., 'published.ymax0.portfolios')"),
        }),
    }
}