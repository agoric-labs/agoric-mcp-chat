...
const chainSchema_staking = z.union([z.literal("osmosis"), z.literal("noble")]).describe("Chain name");
...
const nobleAccountSchema = z.string().startsWith("noble1").describe("Noble account address");

const ethAccountSchema = z.string().startsWith("0x").describe("Ethereum account address");

const agoricAccountSchema = z.string().startsWith("agoric1").describe("Agoric account address");
...
export const agoricMcpToolSchemas = {
    ...
    'propose-spend-action': {
        inputSchema: z.object({
            spendActionB64: z.string().describe(
    ...
    'set-network': {
        inputSchema: z.object({
            agoricNet: z
                ...
                .describe('Network to set ("local" will not work with remote MCP server'),
        }),
    },
